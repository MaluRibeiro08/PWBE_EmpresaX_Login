...
{    
    ...
    //envia o conteúdo para o arquivo
        file_put_contents("./dados/empresaX.json", $jsonFinal);
...
function realizarLogin($usuario, $senha, $dados)
{
    foreach ($dados as $dado) 
    {
        ...
        }*/ //Não da pra usar aqui, mas com BCD sim
    }
...
// FUNCAO DE VERIFICAÇÃO DE LOGIN (usuário passou pelo login?)
function verificarLogin()
{
    if ($_SESSION["id"] != session_id() || empty($_SESSION["id"]))
    {
        /*verifica se o id desta sessão é o mesmo qe guardamos ao fazer 
        o login. Se não for, ou se este ultimo não existir, ela manda ele fazer login*/
        header("location: index.php");
    }
    else {}
}

//FUNCAO DE FINALIZAÇÃO DE LOGIN

function finalizarLogin()
{
session_unset(); //limpa as variáveis de sessões 
